"""Offline Candidate B representation only. No embedding or serving imports."""
...
BASELINE = ROOT / "reports/phase5/phase5a0/baseline_a_manifest.json"
CONTRACT = ROOT / "evals/fixtures/phase5a0_abc_contract.json"
...
class ShadowError(RuntimeError):
    """Fail closed on an invalid source, identity, or representation."""
...
def install_offline_guard():
    """Install before optional imports; this CLI forbids even loopback I/O."""
...
class MiniLMTokenizer:
    ...
    def __init__(self, snapshot=None):
        ...
        embedding = json.loads(BASELINE.read_text())["embedding"]
        revision = embedding["local_resolved_revision"]
        snapshot = (
            Path(snapshot)
            if snapshot
            else ROOT / "evals/fixtures/phase5a1_minilm_tokenizer"
        )
        for name, expected in embedding["tokenizer_file_sha256"].items():
            path = snapshot / name
            if not path.is_file() or sha(path.read_bytes()) != expected:
                raise ShadowError(f"frozen tokenizer unavailable or changed: {name}")
        ...
        if self.count("hello", specials=True) - self.count("hello") != 2:
            raise ShadowError("expected exactly two MiniLM special tokens")
...
class Source:
    def __init__(self, raw):
        ...
        # markdown-it normalizes CRLF/CR to LF internally; maps retain line indices.
        # Only CR/LF are Markdown line boundaries (not Unicode paragraph separators).
    ...
    def span(self, start, end, role="content"):
        if not 0 <= start < end <= len(self.text):
            raise ShadowError(f"invalid source span: {start}:{end}")
        return {
