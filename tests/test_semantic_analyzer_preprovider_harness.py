...
def test_preflight_reports_ready_at_required_baseline():
...
def test_wrong_p6_support_fails(pack: dict):
    ...
    result = qualify_case_response(case_id="P6", bundle=_bundle(pack, "P6"), raw_response=_raw(obs))
...
def test_wrong_p7_support_fails(pack: dict):
    ...
    result = qualify_case_response(case_id="P7", bundle=_bundle(pack, "P7"), raw_response=_raw(obs))
    assert result["disposition"] == "FAIL"
...
def test_p1_wrong_but_in_bounds_support_fails(pack: dict):
    ...
    result = qualify_case_response(case_id="P1", bundle=_bundle(pack, "P1"), raw_response=_raw(obs))
    assert result["disposition"] == "FAIL"
...
def test_markdown_fenced_response_parses(pack: dict, gold_responses: dict):
    fenced = f"```json\n{gold_responses['P1']}\n```"
    parsed = parse_analyzer_response(fenced)
...
def test_altered_draft_with_caller_hash_mismatch_invalid(pack: dict, gold_responses: dict):
    bundle = _bundle(pack, "P6")
    bundle["draft_text"] = bundle["draft_text"] + " mutated"
    bundle["draft_sha256"] = "deadbeef" * 8
    result = qualify_case_response(
        case_id="P6",
        bundle=bundle,
        raw_response=gold_responses["P6"],
        ingress=validate_trusted_ingress(
            bundle,
            caller_draft_sha256=build_case_bundle(pack, "P6")["draft_sha256"],
        ),
    )
    assert result["disposition"] == "INVALID"
    assert "draft_sha256_mismatch" in result["invalid_reasons"]


def test_claim_text_draft_slice_mismatch_invalid(pack: dict, gold_responses: dict):
    bundle = _bundle(pack, "P6")
    bundle["claims"][0]["claim_text"] = "Different claim text."
    result = qualify_case_response(case_id="P6", bundle=bundle, raw_response=gold_responses["P6"])
    assert result["disposition"] == "INVALID"
    assert "claim_P6.claim.1_text_draft_slice_mismatch" in result["invalid_reasons"]


def test_altered_source_hash_mismatch_invalid(pack: dict, gold_responses: dict):
    bundle = _bundle(pack, "P6")
    bundle["evidence_manifest"][0]["source_text"] += "x"
    result = qualify_case_response(case_id="P6", bundle=bundle, raw_response=gold_responses["P6"])
    assert result["disposition"] == "INVALID"
    assert any("source_sha256_mismatch" in reason for reason in result["invalid_reasons"])
...
def test_invalid_envelope_verified_p1_not_promoted(pack: dict, gold_responses: dict):
    ...
    result = qualify_case_response(case_id="P1", bundle=bundle, raw_response=gold_responses["P1"])
    assert result["disposition"] == "INVALID"
...
def test_stale_pass_artifact_rejected(pack: dict, gold_responses: dict):
    artifact = run_offline_qualification(pack, response_provider=gold_responses)
    artifact["identity_hashes"]["implementation_sha"] = "0" * 40
    integrity = verify_artifact_integrity(artifact)
    assert integrity["valid"] is False
    assert "stale_implementation_sha" in integrity["invalid_reasons"]
...
def test_identity_hashes_include_schema_and_prompt():
    hashes = build_run_identity_hashes(implementation_sha=REQUIRED_ENGINE_BASELINE_SHA)
